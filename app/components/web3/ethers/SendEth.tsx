...
import { useEthersSendEth } from '@/app/hooks/ethers/useEthersSendEth';
import { useEthersWaitForTransactionReceipt } from '@/app/hooks/ethers/useEthersWaitForTransactionReceipt';

export default function SendEth() {
  const [to, setTo] = useState('');
  const [value, setValue] = useState('');

  const { sendEth, isPending, error, hash } = useEthersSendEth();
  const { isLoading: isConfirming, isSuccess } = useEthersWaitForTransactionReceipt(hash as `0x${string}`);
